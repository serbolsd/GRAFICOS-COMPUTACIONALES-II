...
void CMesh::loadText(ID3D11Device* m_pd3dDevice)
{
	...
	D3DX11CreateShaderResourceViewFromFile(m_pd3dDevice, direcTextur, NULL, NULL, &textures.m_Difuse.m_pTextureRV, NULL);

	//D3DX11CreateShaderResourceViewFromFile(m_device.m_pd3dDevice, L"miku.dds", NULL, NULL, &m_texture.m_pTextureRV, NULL);
	//if (FAILED(hr))
	//	return hr;
}
void CMesh::loadNormalText(ID3D11Device* m_pd3dDevice)
{
	...
	char *token = NULL;
	char *nextToken = NULL;
	token = strtok_s((char *)norName.c_str(), ".", &nextToken);
	string pngextencion = token;
	pngextencion += ".png";
	token = (char*)pngextencion.c_str();
	...
	D3DX11CreateShaderResourceViewFromFile(m_pd3dDevice, direcTextur, NULL, NULL, &textures.m_Normal.m_pTextureRV, NULL);
